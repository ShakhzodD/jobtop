...
export async function createJob(draft: JobDraft) {
  ...
  if (!initData) throw new Error("E’lon berish Telegram Mini App ichida ishlaydi");
  const response = await fetch("/api/jobs", { method: "POST", headers: { "Content-Type": "application/json", "x-telegram-init-data": initData }, body: JSON.stringify(draft) });
