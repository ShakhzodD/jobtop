...
export async function addUserRole(userId: string, role: UserRole) {
  const { error } = await createSupabaseServerClient().from("user_roles").upsert({ user_id: userId, role });
  if (error) throw error;
}

export async function setActiveUserRole(userId: string, role: UserRole) {
  const supabase = createSupabaseServerClient();
  const { data: roleRow, error: roleError } = await supabase.from("user_roles").select("role").eq("user_id", userId).eq("role", role).maybeSingle();
...
export async function getUserRoles(userId: string): Promise<UserRole[]> {
  const { data, error } = await createSupabaseServerClient().from("user_roles").select("role").eq("user_id", userId);
  if (error) throw error;
